...
class PlatformstatisticsController extends AuthController {
	...
	public function export(){
		...
		$sheet->setTitle($args['year'].'-'.$args['month'].'-'.'代理数据统计信息 - '.$subTitle);
		//平台统计
		if($args['nav'] == 0){
			...
			$execl->getActiveSheet()->getStyle( 'A2:H2')->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID);
			$execl->getActiveSheet()->getStyle( 'A2:H2')->getFill()->getStartColor()->setARGB('FF00CC33');
		}
		...
		$name = iconv("utf-8", "gb2312", $args['year'].'-'.$args['month'].'-'.'代理数据统计信息 - '.$subTitle);
