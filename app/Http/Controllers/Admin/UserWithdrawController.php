 ...
 * 商家提现
 */
class UserWithdrawController extends AuthController {
	...
	public function dispose() {
	    $result = $this->requestApi('user.paylog.dispose', Input::all());
		/*返回处理*/ 
		if($result['code']==0){
	...
	public function export(){
		...
		do {
			...
			foreach ($result['data']['list'] as $key => $value) {
				$sheet->setCellValue('A' . $i, $value['user']['id']);
				$sheet->setCellValue('B' . $i, $value['user']['name']);
		...
		$name = iconv("utf-8", "gb2312", "会员提现信息");
