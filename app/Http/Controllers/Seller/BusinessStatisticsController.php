...
class BusinessStatisticsController extends AuthController {
	...
	public function monthaccountexport(){
		...
		$execl->getActiveSheet()->getStyle( 'A2:L2')->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID);
		$execl->getActiveSheet()->getStyle( 'A2:L2')->getFill()->getStartColor()->setARGB('FF00CC33');
		...
		$sheet->setCellValue('G' . $i, number_format($sum['sendFee'], 2));
		...
		$sheet->setCellValue('K' . $i, number_format($sum['activityNewMoney']+$sum['systemFullSubsidy']+$sum['totalDiscountFee']+$sum['totalIntegralFee'], 2));
		...
		foreach ($result['data']['list'] as $key => $value) {
			...
			$sheet->setCellValue('G' . $i, $value['sendFee']);
			...
			$sheet->setCellValue('K' . $i, number_format($value['systemFullSubsidy'], 2));
