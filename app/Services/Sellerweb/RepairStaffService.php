...
class  RepairStaffService extends \YiZan\Services\RepairStaffService
{
	...
	public static function getSellerList($sellerId, $name, $mobile, $type = '', $page, $pageSize) {
        ...
        if($list){
            $list = $list->toArray();
            foreach($list as $k => $v){
                $list[$k]['repairName'] = RepairType::where('id',$v['repairTypeId'])->pluck('name');
            }
        }else{
            $list = null;
        }
     ...
     * @return array   添加结果
     */
    public static function saveStaff($id, $sellerId, $mobile, $pwd, $name, $avatar,$type,$repairNumber,$repairTypeId,$sex,$status) {
        ...
        try {
            ...
            $staff->repair_number      = $repairNumber;
            $staff->repair_type_id          = $repairTypeId;
            $staff->sort               = 100;
            $staff->status             = $status;
...
//            }
    ...
    public static function getRepair(){
        $result = RepairType::select('id','name')->get()->toArray();
        return $result;
    }
