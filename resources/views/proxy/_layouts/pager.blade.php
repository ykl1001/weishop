<div class="m-fyct clearfix">					
	<div class="m-fycon pt15 mr20 fr">
        @if ($pager['page'] > 1) 
        <a href="{{ $pager['page_prev'] }}" page="{{ $pager['prev_page'] }}" class="prec"><i class="fa fa-angle-double-left"></i></a>
        @endif
        @foreach ($pager['page_nums'] as $page_num) 
            ...
            @else
            <a href="{{ $page_num['url'] }}" page="{{ $page_num['name'] }}" class="num">{{ $page_num['name'] }}</a>
            @endif
        @endforeach
        @if ($pager['page'] < $pager['page_count']) 
        <a href="{{ $pager['page_next'] }}" page="{{ $pager['prev_page'] }}" class="next"><i class="fa fa-angle-double-right"></i></a>
        @endif
    </div>
