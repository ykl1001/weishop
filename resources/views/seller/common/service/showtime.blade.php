...
<script>
    var delurl = "{{ u('SellerService/deldatatime') }}";
    var gettimes = "{{ u('SellerService/gettimes') }}";    
    var showtime = "{{ u('SellerService/showtime') }}";    
    var goodsId = "{{$data['id']}}";
     $(document).on('click','.f-delet',function() {
        ...
        $.post(delurl,{id:id,goodsId:goodsId},function(result){
            if(result.code == 0){ 
    ...
    $(document).on('click','.f-edit',function() { 
        ...
        var id = $(this).data('id');
        $(".data-id").text(id);
        $.post(showtime,{id:id,goodsId:goodsId},function(result){
            if(result.code == 0){
                result = result.data;
