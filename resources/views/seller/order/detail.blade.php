		...
		<div class="m-zjgltbg">
			<div class="p10">						
				<p class="f-bhtt f14 clearfix">
					<span class="ml15 fl">商品类订单详情</span>
                    @if($data['orderType'] == 1)
                ...
                @endif
                @if($data['isCanStartService'] && $data['status'] != ORDER_STATUS_SYSTEM_SEND && $data['status'] != ORDER_STATUS_CALL_SYSTEM_SEND && $data['status'] != ORDER_STATUS_GET_SYSTEM_SEND)
                    <a href="javascript:;" class="fr btn mb10 hsbtn-78 mt10 ml20" onclick="$.orderStart(1)">开始配送</a>
                @endif
                @if($data['isCanFinish'] && $data['status'] != ORDER_STATUS_SYSTEM_SEND && $data['status'] != ORDER_STATUS_CALL_SYSTEM_SEND && $data['status'] != ORDER_STATUS_GET_SYSTEM_SEND)
                    <a href="javascript:;" class="fr btn mb10 hsbtn-78 mt10 ml20" onclick="$.orderFinish(1)">完成订单</a>
                @endif
                @if($data['status'] == ORDER_STATUS_AUTH_CODE && !empty($data['authCode']) && ($data['sendWay'] == 2 || $data['sendWay'] == 3))
                    <a href="javascript:;" class="fr btn mb10 hsbtn-78 mt10 ml20" onclick="$.orderFinish(1)">完成消费</a>
                @endif

				
                @if($data['orderType'] == 1)
                    @if($data['isCanCall'] && $data['seller']['sendType'] == 2 && $data['sellerFee'] >= $system_send_staff_fee && ($data['sendWay'] == 1 || $data['sendWay'] == 0))
                        <a href="javascript:;" class="fr btn mb10 hsbtn-78 mt10 ml20 button f12" onclick="$.isCanCall()" style="width:100px;">呼叫平台配送</a>
                        <p class="f_aaa f12 fr" style="clear:both;">使用{{$site_name}}配送, 仅需{{$system_send_staff_fee}}元</p>
                    @endif
                    @if($data['isCanCancelCall'])
                        <a href="javascript:;" class="fr btn mb10 hsbtn-78 mt10 ml20 button f12" onclick="$.isCanCancelCall()" style="width:120px;" class="button f12">取消呼叫平台配送</a>
                    @endif
                @endif
        					...
        					<table>
        						<tr>
        						    <td style="padding-left:10px">
										...
										<p class=" f14">配送地址：{{ $data['name'] }}，{{ $data['mobile'] }}，{{$data['province']}}{{$data['city']}}{{$data['area']}}{{ $data['address'] }}</p>
										<p class=" f14">配送时间：{{ $data['appTime']}}</p>
								...
								<tr>
        						    <td style="padding-left:10px">
                                        @if($data['status'] == ORDER_STATUS_CALL_SYSTEM_SEND || $data['status'] == ORDER_STATUS_SYSTEM_SEND)
                                            <p class=" f14">配送人员：待平台指派
                                        @else
                                            <p class=" f14">配送人员：{{ $data['staff']['name']}}    {{ $data['staff']['mobile']}}
                                        @endif
	        						    @if( $data['status'] < ORDER_STATUS_FINISH_STAFF && $data['status'] != ORDER_STATUS_CALL_SYSTEM_SEND && $data['status'] != ORDER_STATUS_GET_SYSTEM_SEND && $data['status'] != ORDER_STATUS_SYSTEM_SEND && ($data['sendWay'] == 1 || $data['sendWay'] == 0))
...
<script type="text/tpl" id="cancel_tips">
    <div style="padding:15px;">每个订单只能取消一次，您确认取消平台配送吗？</div>
</script>
...
var id = {{$data['id']}};
        ...
        //呼叫配送
        $.isCanCall = function () {
            $.refundRemark({{ORDER_STATUS_CALL_SYSTEM_SEND}},1);
        }
        //取消呼叫配送
        $.isCanCancelCall = function () {
            var dialog = $.zydialogs.open($("#cancel_tips").html(), {
                boxid:'SET_GROUP_WEEBOX',
                width:300,
                title:'取消呼叫配送',
                showClose:true,
