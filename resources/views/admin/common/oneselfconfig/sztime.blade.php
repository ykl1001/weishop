...
<script>
    var szurl = "{{ u('OneselfConfig/updatetime') }}";
    var adddatatime = "{{ u('OneselfConfig/addtime') }}";
    to_html ();
    //times ();
    alod ();
    function times () {
    ...
    $(".m-sjglbcbtn").click(function(){
        ...
        var sellerId = $("input[name=id]").val();
        obj = new Object();
        obj.weeks = week;
        obj.hours = hour;
        ...
        if($(this).text() == "更新"){
                ...
                $.post(adddatatime,obj,function(result){  
                    if(result.status == true){
                        ...
                        $.ShowAlert(result.msg);
                        $.post(gettimes,{id:obj.sellerId},function(res){
                            var htmls = "";
                            if(res.code == 0){                  
