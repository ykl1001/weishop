	   ...
	   <div @if($data['status'] == 0) style="display:none;"@endif> -->
		<yz:list>
			<tabs>
				<navs>
				    <nav label="抢购设置">
						<attrs>
							<url>{{ u('ShoppingSpree/setting',['id'=>$data['id']]) }}</url>
							<css> @if(ACTION_NAME == 'setting') on @endif </css>
						</attrs>
					</nav>
					<nav label="抢购活动">
						<attrs>
							<url>{{ u('ShoppingSpree/index',['id'=>$data['id']]) }}</url>
		...
		<yz:form id="yz_form" action="save">
    		<div class="form-tip">
				...
				<yz:fitem label="状态">
				    <yz:radio name="status" options="1,0" texts="开启,关闭" checked="$data['status']"></yz:radio>
				</yz:fitem>
