            ...
            <div class="m-tab m-smfw-ser pt20">
                ...
                @yizan_begin
                    <yz:form nobtn="1">
                        ...
                        @endif
                        <yz:fitem name="owner" label="姓名" type="text">
                            {{ $data['puser']['name'] }}
                        </yz:fitem>

                        <yz:fitem name="owner" label="认证身份" type="text">
                            {{ $type[$data['puser']['type']] }}
                        </yz:fitem>
                        ...
                        @if($data['star']&& $data['rateContent'])

                        <div class="u-rtt f3f6fa">
                            <i class="fa fa-credit-card mr15 ml15"></i>维修评论
                        </div>

                        <yz:fitem name="rateContent" label="评论星级" type="text">
                            <p><!-- {{ $data['star'] }} --> 
                               @if($data['star'])
                               @for($i = 0;$i<$data['star'];$i++)<img src="{{asset('images/ico/pstar.png')}}" width="22">@endfor
                               @endif
                            </p>
                        </yz:fitem>
                        <yz:fitem name="rateContent" label="评论详情" type="text">
                            <p>  {{ $data['rateContent'] }}</p>
                        </yz:fitem>
                        @endif
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        $('.dispose').click(function() {
            ...
            $.post("{{ u('Property/designate')  }}",{'staffId':staffId,'id':id,'status':status,'sellerId':sellerId},function(result){
                if(result.code == 0){
                    $.ShowAlert('操作成功');
                    window.location.reload();
                }else{
