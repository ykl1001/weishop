...
@yizan_begin 
		<yz:list>
    		<search url="{{ $url }}">
    			<row>
    				<item name="name" 	label="配送人员名称"></item>
			...
			<btns>
				<linkbtn label="导出到EXCEL" type="export">
					<attrs>
						<url>{{ u('SellerStaffWithdraw/export', $search_args ) }}</url>
					</attrs>
				</linkbtn>
			</btns>
            <yz:tabs>
                <navs>
                    <nav name="status" label="待处理">
                        <attrs>
                            <url>{{ u('SellerStaffWithdraw/index',['status'=>'0','nav'=>1]) }}</url>
                            <css>@if( $nav == 1) on @endif</css>
                        </attrs>
                    </nav>
                    <nav name="status" label="已通过">
                        <attrs>
                            <url>{{ u('SellerStaffWithdraw/index',['status'=>'1','nav'=>2]) }}</url>
                            <css>@if( $nav == 2 ) on @endif</css>
                        </attrs>
                    </nav>
                    <nav name="status" label="已拒绝">
                        <attrs>
                            <url>{{ u('SellerStaffWithdraw/index',['status'=>'2','nav'=>3]) }}</url>
			...
			<table>
				...
				<column label="提现信息" code="seller" align="left" width="180">
                    <p>配送人员名称：{{ $list_item['staff']['name'] }}</p>
                    <p>配送人员余额：￥{{ $list_item['extend']['withdrawMoney'] }}</p>
				...
				<column label="状态" code="status" width="40">
					...
					<actions width="80">
                        <action label="提现记录" >
                            <attrs>
                                <url>{{ u('SellerStaffWithdraw/index',['status'=>'1','nav'=>2,'name'=>$list_item['staff']['name']])}}</url>
                            </attrs>
                        </action>   
                        <br>  				 
        ...
        onOk: function(){
        	...
        	query.content = $("#disposeRemark").val();
                if(type == "-1"){ //拒绝
                	...
                	$.post("{{ u('SellerStaffWithdraw/dispose')  }}",query,function(result){
