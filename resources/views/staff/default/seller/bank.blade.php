...
@section('show_top')
    <header class="bar bar-nav">
        <a class="button button-link button-nav pull-left" href="#" onclick="JumpURL('{{ u('Seller/'.$url) }}','#seller_{{$url}}_view',2)" data-transition='slide-out'>
...
@section('content')
   <form  method="post"  id="ajax-form">
       ...
       @else
           @if(!$verifyCode)
               <div class="y-plr10">
                   <a href="#" onclick="JumpURL('{{u('Seller/verifyCode',['id'=>$data['id']])}}','#seller_verifyCode_view',2)" class="button button-fill button-danger  bg_ff2d4b">编辑</a>
               </div>
           @else
    ...
    <script type="text/javascript">
        $(function(){
            $("#{{$id_action.$ajaxurl_page}} #ajax-form").submit(function(){
                $.post("{{u("Seller/bankSve")}}", $(this).serialize(), function(result){
                    if(result.code == 0){
                        var url ="{{$url}}";
                        if(url == "index"){
                            JumpURL('{{u('Seller/bank',['id'=>$data['id']])}}','#seller_bank_view',1);
                        }else{
                            JumpURL('{{u('Seller/carry')}}','#seller_carry_view',2);
                        }
                    }else{
                        $.toast(result.msg);
                    }
