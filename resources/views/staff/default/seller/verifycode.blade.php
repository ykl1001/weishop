...
@section('show_top')
    <header class="bar bar-nav">
        <a class="button button-link button-nav pull-left" href="#" onclick="JumpURL('{{ u('Seller/bank',['id'=>$data['id']]) }}','#seller_index_view',2)" data-transition='slide-out'>
    ...
    <script type="text/javascript">
        $(function(){
            $("#{{$id_action.$ajaxurl_page}} #verifyCode_show").click(function(){
                ...
                $.post("{{u('Seller/verifyCodeCk')}}", {mobile:mobile,verifyCode:verifyCode}, function(result){
                    var url = "{!!u('Seller/bank',['id'=>$data['id']])!!}&verifyCode="+result.data;
                    if(result.code == 0){
