@foreach($list['orders'] as $v)
    <div class="card y-ddcard z-gray">
        ...
        <div class="card-content" onclick="JumpURL('{{u('Order/detail',['id'=>$v['id']])}}','#order_detail_view',2)">
        ...
        @if($v['isPay'] || $v['isCanAccept'] || $v['isLogistics'])
            ...
            <span class="y-ddlistbtn fr">
                @if($v['isCancfOrder'] && in_array($role,[1,3,5,7]) )
                    <a href="#" onclick="$.isCancfOrder({{$v['id']}})" class="y-viewlogistics">确定取消</a>
                @endif
                @if($v['isPay'])
                ...
                @endif
                @if($v['isLogistics'])
                    <a href="#" onclick="JumpURL('{{u('Order/logistics',['id'=>$v['id']])}}','#order_logistics_view',2)" class="y-viewlogistics">查看物流</a>
                @endif
            </span>
            </div>
        @endif
